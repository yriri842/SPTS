...
UI.CancelBtn.Active                  = true
UI.CancelBtn.Selectable              = true
UI.CancelBtn.AutoButtonColor         = false
UI.CancelBtn.ZIndex                  = 10001
...
local function doCancel()
    ...
    task.delay(0.4, function()
        ...
        if _G.SPTS_Unload then
            task.spawn(function() pcall(_G.SPTS_Unload) end)
        end
    end)
end

UI.CancelBtn.MouseButton1Click:Connect(doCancel)
UI.CancelBtn.Activated:Connect(doCancel)
...
local function setStatus(text)
    task.spawn(function()
        if not UI.Status or not UI.Status.Parent then return end
